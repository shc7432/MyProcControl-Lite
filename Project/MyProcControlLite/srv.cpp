...
void WindowsService::ServiceCoreThread() {
	...
	while (!m_stopRequested) {
		...
		for (size_t index = 0, size = InjectHelperProcess.size(); index < size; ++index) {
			...
			if (i) CloseHandle(i);
			bool isX86 = index == 0;
			wstring rundll32 = isX86 ? RunDLL_X86 : RunDLL_X64;
			wstring dllfile = isX86 ? injector86 : injector64;
			STARTUPINFOW si{ sizeof(si) }; PROCESS_INFORMATION pi{};
			si.dwFlags = STARTF_USESTDHANDLES;
			if (isX86) {
				if (injector86_in) { CloseHandle(injector86_in); injector86_in = NULL; }
				if (injector86_out) { CloseHandle(injector86_out); injector86_out = NULL; }
			}
			else {
				if (injector64_in) { CloseHandle(injector64_in); injector64_in = NULL; }
				if (injector64_out) { CloseHandle(injector64_out); injector64_out = NULL; }
			}
			SECURITY_ATTRIBUTES caninherit{ .nLength = sizeof(caninherit), .lpSecurityDescriptor = NULL, .bInheritHandle = TRUE };
			if (!CreatePipe(&(isX86 ? injector86_in : injector64_in), &(isX86 ? injector86_out : injector64_out), &caninherit, 0)) {
				m_status.dwControlsAccepted = 0;
				m_status.dwWin32ExitCode = GetLastError();
				ReportStatus(SERVICE_STOPPED);
				ExitProcess(m_status.dwWin32ExitCode);
			...
			si.hStdInput = (isX86 ? injector86_in : injector64_in);
			si.hStdError = si.hStdOutput = (isX86 ? injector86_out : injector64_out);
			wstring cmd = L"RunDLL32 \"" + dllfile + L"\",RunDLL /=help[] /password=0812 " + 
				to_wstring((ULONG_PTR)si.hStdOutput) + L" /ppid=" + to_wstring(GetCurrentProcessId());
